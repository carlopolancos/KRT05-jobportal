...
server.tool('search_karate_features', 'Search and retrieve Karate DSL features by category', {
    ...
    if (categoryData.tips) {
        ...
        categoryData.tips.forEach((tip) => { result += `- ${tip}\n`; });
    }
...
server.tool('get_karate_feature', 'Get detailed information about a specific Karate feature', {
    ...
    for (const [category, data] of Object.entries(karateFeatures)) {
        if (data.features) {
            const f = data.features.find((x) => x.name.toLowerCase().includes(term));
            if (f) {
                found = f;
                cat = category;
                break;
        ...
        if (data.functions) {
            const f = data.functions.find((x) => x.name.toLowerCase().includes(term));
            if (f) {
                found = f;
                cat = category;
                break;
    ...
    if (!found)
        return { content: [{ type: 'text', text: `Feature '${featureName}' not found.` }] };
...
server.tool('get_karate_best_practices', 'Get Karate best practices', {}, async () => {
    ...
    p.tips.forEach((t, i) => { result += `${i + 1}. ${t}\n`; });
    return { content: [{ type: 'text', text: result }] };
});
